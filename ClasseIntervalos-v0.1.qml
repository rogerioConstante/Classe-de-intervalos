...
MuseScore {
      menuPath: "Plugins.Contraponto.ClasseIntervalos"
      description: "Classe de Intervalos.\nPlugin para analisar e escrever os intervalos em exercícios de contraponto."
      version: "0.1"

      //4.4 title: "Classe de Intervalos"
      //4.4 thumbnailName: "Intervalos.png"
      //4.4 categoryCode: "Contraponto"

      Component.onCompleted: {
        if (mscoreMajorVersion >= 4) {
            title = qsTr("Classe de Intervalos")
...
function mostraIntervalos() {
 ...
 for (var voz1=0;voz1<vozes[1].nota.length;voz1++) { // percorre vozes - 1
  ...
  if (vozes[x].nota[voz1] == 1000 || vozes[x].nota[voz2] == 1000) { var int1 = "erro"; }
        ...
        if (int2 < 0) { var int1 = int2pc(int2*-1); int2 = int2 * -1; }
             else { var int1 = int2pc(int2); };// classe de intervalos

     // calcula intervalos compostos e indica qtd oitavas através de '. 12ªJ = 5'J; 17ªM = 3''M.
  /*  if (int2 > 12) {
      if (int2 < 127) {
...
//console.log(cursor.tick);
    ...
    if (x == 0 && vozes[1].nota.length > 2) { var nome = newElement(Element.STAFF_TEXT); // indica entre quais vozes são os intervalos, antes do primeiro intervalo
                  nome.text = "(" + (voz1+1) + " e " + (voz2+1) + "):";
                  nome.offsetX = - 6;
                  nome.offsetY = 3 + extraY;
                  nome.placement = 1;
                  nome.autoplace = false;
                  cursor.add(nome); };

    var myText = newElement(Element.STAFF_TEXT);
        if ((!int1 && int1 !== 0) || int1 == "erro") { myText.text = ""; } else { myText.text = int1;};
        ...
        if (vozes[1].nota.length == 2) { myText.offsetY = 5 + extraY; } else { myText.offsetY = 3 + extraY; }
        myText.placement = 1;
